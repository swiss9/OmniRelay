...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Inter', system-ui, -apple-system, sans-serif; background: #000; color: #fff; min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 2rem; }
    .card { background: #0a0a0a; border-radius: 2.5rem; border: 1px solid #222; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.8); padding: 3rem 2.5rem; max-width: 520px; width: 100%; text-align: center; }
    .logo { width: 90px; height: auto; margin-bottom: 1.5rem; filter: drop-shadow(0 0 10px rgba(255,255,255,0.2)); }
    h1 { font-size: 3rem; font-weight: 800; letter-spacing: -0.02em; color: #fff; margin-bottom: 0.5rem; }
    .tagline { font-size: 1.15rem; color: #a0a0a0; margin-bottom: 2.5rem; }
    .btn { display: inline-block; padding: 1rem 2.5rem; background: #fff; color: #000; font-weight: 700; font-size: 1rem; border-radius: 50px; text-decoration: none; box-shadow: 0 4px 14px rgba(255,255,255,0.15); margin-bottom: 2rem; }
    .github-link { display: inline-flex; align-items: center; gap: 0.5rem; color: #888; text-decoration: none; font-size: 0.9rem; border: 1px solid #333; padding: 0.65rem 1.4rem; border-radius: 50px; margin-bottom: 1.5rem; }
...
<body>
  <div class="card">
    ...
    <div class="footer-note">Already have a license? <a href="#">Configure your extension</a>.<br>Payments in USDT.</div>
  </div>
</body>
</html>`);
});
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    h1 { font-size: 2rem; font-weight: 800; color: #fff; margin-bottom: 0.4rem; }
    ...
    .network-card { background: #0d0d0d; border: 1px solid #2a2a2a; border-radius: 1rem; padding: 1rem 0.5rem; text-align: center; cursor: pointer; font-weight: 600; color: #999; font-size: 0.9rem; }
    .network-card.active { border-color: #fff; background: #141414; color: #fff; }
    .payment-details { background: #0d0d0d; border: 1px solid #2a2a2a; border-radius: 1rem; padding: 1.5rem; margin-bottom: 1.5rem; text-align: left; }
    .address-box { background: #050505; border: 1px solid #333; border-radius: 10px; padding: 0.8rem 1rem; font-family: monospace; word-break: break-all; color: #fff; font-size: 0.85rem; }
    button { width: 100%; padding: 1rem; background: #fff; color: #000; font-weight: 700; border: none; border-radius: 50px; cursor: pointer; margin-top: 0.5rem; }
    #result { margin-top: 1.5rem; padding: 1rem; border-radius: 12px; display: none; font-weight: 500; }
    .success { background: #0a2a1a; color: #4ade80; }
...
<body>
  <div class="card">
    ...
    <div id="paymentDetails" class="payment-details" style="display:none;">
      <div class="detail-row">
        ...
        <small id="contractInfo" style="display:none;">Contract: <span id="contract"></span></small>
      </div>
      <div class="detail-row">
        <label>Transaction Hash (TXID)</label>
        <input type="text" id="txHash" placeholder="Paste your transaction hash" style="width:100%; padding:0.9rem; background:#050505; border:1px solid #333; border-radius:10px; color:#fff;">
      </div>
    </div>
  ...
  <script>
    ...
    function updatePaymentDetails() {
      ...
      document.getElementById('contractInfo').style.display = info.usdtContract ? 'block' : 'none';
    }
